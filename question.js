export const allQuestions = [
    {
        number: 1,
        question: "Bir duyguyu, düşünceyi, bilgiyi, haberi bildirmek veya yaymak için en az iki kişi arasında çeşitli yollarla gerçekleşen olgu aşağıdakilerden hangisidir?",
        options: ["Kitle", "Çevre", "Toplum", "İletişim", "Sanat"],
        answer: "İletişim"
    },
    {
        number: 2,
        question: "İşaretler, semboller, resimler gibi görüntü merkezli bir dil kullanan ve evrensel bir ifade biçimi olarak tanımlanan iletişim türü aşağıdakilerden hangisidir?",
        options: ["Sözel iletişim", "İşitsel iletişim", "Yazılı iletişim", "Duyusal iletişim", "Görsel iletişim"],
        answer: "Görsel iletişim"
    },
    {
        number: 3,
        question: "Gerçeğe en yakın formlardan biridir, duyguları harekete geçirir, bilgiyi bellekte saklamayı kolaylaştırır. Yukarıdakilerden hangisi veya hangileri fotoğrafın özellikleri arasında yer almaktadır?",
        options: ["Yalnız I", "Yalnız II", "Yalnız III", "II ve III", "I, II ve III"],
        answer: "II ve III"
    },
    {
        number: 4,
        question: "MÖ 8. yüzyıl sonlarında, özellikle denize kıyısı olan ülkelerdeki tüccarların kendi aralarında tanıtıcı işaretler olarak kullandıkları sembollere ne ad verilmektedir?",
        options: ["Amblem", "Flama", "Bayrak", "Afiş", "Resimli bez"],
        answer: "Amblem"
    },
    {
        number: 5,
        question: "Fotoğraf, özgürlüklerin kazanılması mücadelesinde ön ayak olmuş, modern çağı sembolize eden tipik bir iletişim aracı haline gelmiştir. Fotoğrafa ilişkin bu ifade onun hangi işlevini vurgulamaktadır?",
        options: ["Duygulandırıcı işlevi", "Tanıtıcı işlevi", "Demokratik işlevi", "Eğlendirme işlevi", "Sanatsal işlevi"],
        answer: "Demokratik işlevi"
    },
    {
        number: 6,
        question: "Kültür, tarih, doğa, yaşam biçimleri, sanat gibi insanı ilgilendiren birçok konu hakkında bilgi aktarmayı ve ele alınan konuyu belgeleriyle ortaya koymayı hedefleyen fotoğraf türü aşağıdakilerden hangisidir?",
        options: ["Sanat fotoğrafı", "Belgesel fotoğraf", "Soyut fotoğraf", "Portre fotoğrafı", "Reklam fotoğrafı"],
        answer: "Belgesel fotoğraf"
    },
    {
        number: 7,
        question: "Fotoğrafın hedef kitleye en fazla sayıda erişimini sağlamak ve görsel etkiyi arttırıp zihinde bir görüntü oluşturmak amacıyla kullanılan fotoğraf türü aşağıdakilerden hangisidir?",
        options: ["Haber fotoğrafı", "Sanat fotoğrafı", "Reklam fotoğrafı", "Deneysel fotoğraf", "Soyut fotoğraf"],
        answer: "Reklam fotoğrafı"
    },
    {
        number: 8,
        question: "Bir endüstriye dönüşmüştür, kitleseldir, sadece profesyoneller içindir. Yukarıdakilerden hangisi veya hangileri günümüz dünyasında fotoğraf için geçerlidir?",
        options: ["Yalnız I", "Yalnız II", "Yalnız III", "I ve II", "I, II ve III"],
        answer: "I ve II"
    },
    {
        number: 9,
        question: "Fotoğrafçılıkta belirlenmiş veya kendi belirlediği ölçülere göre neyi, ne zaman, nasıl, ne kadar çekeceğine karar vererek fotoğrafı, duygu, düşünce veya fikirlerini insanlara aktarmada bir iletişim yöntemi olarak kullanır. Verilen ifade aşağıda belirtilen hangi fotoğraf türü için geçerlidir?",
        options: ["Spor fotoğrafı", "Haber fotoğrafı", "Reklam fotoğrafı", "Tanıtım fotoğrafı", "Sanat fotoğrafı"],
        answer: "Sanat fotoğrafı"
    },
    {
        number: 10,
        question: "İngiliz filozof ve araştırmacı John Locke’un araştırmalarında, insanların öğrenirken yüzde 83 ile en yüksek oranda kullandıkları yöntem aşağıdakilerden hangisidir?",
        options: ["Duyarak", "Gözlemleyerek", "Deneyerek", "Koklayarak", "Duyarak"],
        answer: "Gözlemleyerek"
    },

    {
        number: 11,
        question: "Fotoğraf kaçıncı yüzyılda ortaya çıkmıştır?",
        options: ["15. yüzyıl", "16. yüzyıl", "17. yüzyıl", "18. Yüzyıl", "19. Yüzyıl"],
        answer: "19. Yüzyıl"
    },
    {
        number: 12,
        question: "Karanlık kutu anlamına gelen kavram aşağıdakilerden hangisidir?",
        options: ["Camera lucida", "Camera obscura", "Fotoğraf makinesi", "Perspektif", "Görüntü çoğaltma"],
        answer: "Camera obscura"
    },
    {
        number: 13,
        question: "İbn-i Heysem’in ışığın doğrusal yayılımı hakkında bilgi verdiği deney hangisidir?",
        options: ["Karanlık kutu", "İğne deliği", "Mum deneyi", "Camera obscura", "Helyografi"],
        answer: "Mum deneyi"
    },
    {
        number: 14,
        question: "Fotoğrafın kullanım amaçları arasında aşağıdakilerden hangisi yer almaz?",
        options: ["Görüntüleri sabitleme amacıyla", "Görüntüleri çoğaltma amacıyla", "Boyama tekniği amacıyla", "Sanatsal bir anlatım amacıyla", "Bilimsel çalışmalar amacıyla"],
        answer: "Boyama tekniği amacıyla"
    },
    {
        number: 15,
        question: "Sanayi Devrimi ile aşağıdaki buluşlardan hangisi gerçekleşmiştir?",
        options: ["Yazının icadı", "Matbaanın icadı", "Karanlık kutunun icadı", "Fotoğrafın icadı", "Perspektifin bulunuşu"],
        answer: "Fotoğrafın icadı"
    },
    {
        number: 16,
        question: "I. Tarihte ilk kalıcı görüntüyü kaydetmiştir.\nII. Fotoğrafı keşfeden ilk kişidir.\nIII. Yöntemine Helyografi ismini vermiştir.\nYukarıda verilen bilgiler aşağıdaki isimlerden hangisine aittir?",
        options: ["İbn-i Heysem", "Aristoteles", "W. H. Talbot", "L. M. Daguerre", "J. N.Niepce"],
        answer: "J. N.Niepce"
    },
    {
        number: 17,
        question: "Fotoğraf alanında çalışma yapan isimler arasında aşağıdakilerden hangisi yoktur?",
        options: ["Aristoteles", "W. H.Talbot", "L. M. Daguerre", "J. N.Niepce", "T. Wedgwood"],
        answer: "Aristoteles"
    },
    {
        number: 18,
        question: "Daguerre’nin icadı olan Dagurretip yönteminin özellikleri arasında aşağıdakilerden hangisi yer almaz?",
        options: ["Pozlama süresinin kısalması", "Görüntülerin tek kopya olması", "Malzemenin ağır olması", "Malzemenin pahalı olması", "Boyutların küçük olması"],
        answer: "Boyutların küçük olması"
    },
    {
        number: 19,
        question: "H. F. Talbot’un geliştirdiği yöntemin en önemli özelliği aşağıdakilerden hangisidir?",
        options: ["Görüntüleri sabitleme", "Görüntüleri çoğaltma", "Görüntüleri büyütme", "Görüntülerin tek kopya olması", "Görüntülerin renkli olması"],
        answer: "Görüntüleri çoğaltma"
    },
    {
        number: 20,
        question: "Fotoğrafın geçirdiği tarihsel süreci aşağıdakilerden hangisi ifade etmez?",
        options: ["Camera Obscura’nın bulunuşu", "Sanayi Devrimi", "Reform hareketleri", "Görüntünün sabitlenmesi", "Görüntünün çoğaltılması"],
        answer: "Reform hareketleri"
    },

    {
        number: 21,
        question: "Camera Obscura’nın çalışma prensibini Rönesans Dönemi’nde ilk kez tanımlayan isim aşağıdakilerden hangisidir?",
        options: ["Niepce", "Uccello", "Leonardo Da Vinci", "Raffaello", "Michalengelo"],
        answer: "Leonardo Da Vinci"
    },
    {
        number: 22,
        question: "Aynadaki yansıma kuralını bularak görüntünün baş aşağı görülme eksikliğini gideren isim aşağıdakilerden hangisidir?",
        options: ["Johannes Kepler", "Daniello Barbaro", "İbn-i Heysem", "Giovanni Battista Della", "Daguerre"],
        answer: "Johannes Kepler"
    },
    {
        number: 23,
        question: "1826 yılında kalıcı görüntüyü sağlayarak ilk fotoğraf görüntüsünü elde eden isim aşağıdakilerden hangisidir?",
        options: ["François Arago", "Niepce", "William Henry Fox Talbot", "George Eastman", "Joseph Bancroft"],
        answer: "Niepce"
    },
    {
        number: 24,
        question: "Fotoğrafın resmen bir buluş olarak kabul edilmesinin resmî belgesi niteliğindeki buluş aşağıdakilerden hangisidir?",
        options: ["Banyolama", "Calotype", "Fotogram", "Daguerreotype", "Pozlama"],
        answer: "Daguerreotype"
    },
    {
        number: 25,
        question: "İlk kez negatif ve pozitif tekniğini bularak tek bir negatifi çoğaltıp pozitif görüntüler elde etmenin mümkün olduğunu gösteren isim aşağıdakilerden hangisidir?",
        options: ["Niepce", "George Eastman", "François Arago", "Giovanni Battista Della", "William Henry Fox Talbot"],
        answer: "William Henry Fox Talbot"
    },
    {
        number: 26,
        question: "Yüzey üzerinde görüntü oluşmasına yardımcı olan ve ışıkla etkileşimi sonucu kararma-ağarma özelliklerine sahip kimyasal madde aşağıdakilerden hangisidir?",
        options: ["Gümüş", "Bakır", "Altın", "Krom", "Civa"],
        answer: "Gümüş"
    },
    {
        number: 27,
        question: "“Universal tip” olarak bilinen hem gün ışığında hem de yapay ışıkta elde edilen renkli negatifler hangi yöntemle fotoğraf kartlarına tab edilerek birer pozitif görüntüye dönüştürülürler?",
        options: ["Banyolama", "Küçültme", "Büyütme", "Fotogram", "Calotype"],
        answer: "Büyütme"
    },
    {
        number: 28,
        question: "İki boyutlu fotoğraflar yerine üç boyutlu fotoğraflar elde ederek fotoğraflara derinlik hissi veren fotoğraf makinesi türü aşağıdakilerden hangisidir?",
        options: ["Leica Fotoğraf Makinesi", "Katlanabilir Fotoğraf Makinesi", "Dijital Fotoğraf Makinesi", "Stereoskopik Fotoğraf Makinesi", "Kutu Fotoğraf Makinesi"],
        answer: "Stereoskopik Fotoğraf Makinesi"
    },
    {
        number: 29,
        question: "Niepce’nin metal levha üzerinde ilk mimari fotoğrafı elde ederken kullanmış olduğu yöntem aşağıdakilerden hangisidir?",
        options: ["Fotografi", "Gölgeleme", "Heliogarfi", "Calotyp", "Daguerreotype"],
        answer: "Heliogarfi"
    },
    {
        number: 30,
        question: "Kutu fotoğraf makinesinin özellikleri arasında aşağıdakilerden hangisi yoktur?",
        options: ["Fotoğraf makinelerinin en basit halidir.", "Madeni ya da bakalit gövdeden oluşur.", "Objektifleri iç bükey-dış bükey malzemeden yapılmıştır.", "Sabit netleme özelliğine sahiptir.", "Dia pozitif filmler kullanılır."],
        answer: "Dia pozitif filmler kullanılır."
    },

    {
        number: 31,
        question: "Osmanlı’da 1850 yılında ilk yerli stüdyoyu açan fotoğrafçı aşağıdakilerden hangisidir?",
        options: ["Basile Kargopoulo", "Semiha Es", "Cemal Işıksel", "Etem Tem", "Şinasi Barutçu"],
        answer: "Basile Kargopoulo"
    },
    {
        number: 32,
        question: "Osmanlı’da kaliteli fotoğraf-kâğıt-baskı anlamında fotoğrafçılığı profesyonel anlamda uygulayan ve gelişimine katkı sağlayan dergi aşağıdakilerden hangisidir?",
        options: ["Takvim-i Vakayi", "Tercüman-ı Ahval", "Servet-i Fünun", "Tasvir-i Efkar", "Akbaba"],
        answer: "Servet-i Fünun"
    },
    {
        number: 33,
        question: "Osmanlı dönemi fotoğrafçıları arasında aşağıdakilerden hangisi yoktur?",
        options: ["Ferit İbrahim", "Ebuzziye Tevfik", "Velid Ebuzziya", "Ara Güler", "Arif Hikmet Koyunoğlu"],
        answer: "Ara Güler"
    },
    {
        number: 34,
        question: "Osmanlı’nın son dönemlerinde Türk Hanımlar Fotoğrafhanesi’ni kurarak portre fotoğrafları çeken ilk profesyonel kadın fotoğrafçımız aşağıdakilerden hangisidir?",
        options: ["Naciye Suman", "Semiha Es", "Cemile Tan", "Yıldız Kamuran", "Sevil Sert"],
        answer: "Naciye Suman"
    },
    {
        number: 35,
        question: "Cumhuriyet’in ilk yıllarında fotoğrafçılıkta egemen olan Romantik Akım’ın özellikleri arasında aşağıdakilerden hangisi yoktur?",
        options: ["Türkiye’nin tanıtımı hedeflenmiştir.", "İnkılapların yerleşmesine çalışılmıştır.", "Ülkenin güzellikleri fotoğraflanmıştır.", "Manzara fotoğrafçılığı gelişmiştir.", "Sosyal gerçekçi fotoğraflar çekilmiştir."],
        answer: "Sosyal gerçekçi fotoğraflar çekilmiştir."
    },
    {
        number: 36,
        question: "1980 sonrası fotoğrafçılık akımının özellikleri arasında aşağıdakilerden hangisi yoktur?",
        options: ["Kurgusaldır.", "Gerçekçidir.", "Bireyseldir.", "Soyuttur.", "Deneyseldir."],
        answer: "Gerçekçidir."
    },
    {
        number: 37,
        question: "Tanıtım ve reklam fotoğrafçılığının siyasal iletişimden, pazarlamaya hemen her mecrada (billboard, gazete, dergi ve internet) küresel dünyada genişleyen bir alana dönüştüğü dönem aşağıdakilerden hangisidir?",
        options: ["Osmanlı Dönemi", "Romantik Dönem", "Geçiş Dönemi", "1950-1970 Arası Dönem", "1980 Sonrası Dönem"],
        answer: "1980 Sonrası Dönem"
    },
    {
        number: 38,
        question: "Fotoğrafçılık alanında dernekleşme faaliyetlerinin başladığı dönem aşağıdakilerden hangisidir?",
        options: ["Osmanlı Dönemi", "Romantik Dönem", "Geçiş Dönemi", "1950-1970 Arası Dönem", "1980 Sonrası Dönem"],
        answer: "1950-1970 Arası Dönem"
    },
    {
        number: 39,
        question: "Türkiye’de fotoğrafçıların köyden kente göç, şehirleşme, yoksulluk gibi sosyal sorunlara odaklanmaya başladıkları dönem aşağıdakilerden hangisidir?",
        options: ["Osmanlı Dönemi", "Romantik Dönem", "Geçiş Dönemi", "1950-1970 Arası Dönem", "1980 Sonrası Dönem"],
        answer: "1950-1970 Arası Dönem"
    },
    {
        number: 40,
        question: "I. Görüntünün önceden izlenebilmesi\nII. Fotoğraf filminin ortadan kalkması\nIII. Görüntünün anında elde edilebilmesi\nYukarıdakilerden hangisi ya da hangileri dijital fotoğrafçılığın özellikleri arasındadır?",
        options: ["Yalnız I", "Yalnız II", "Yalnız III", "II ve III", "I, II ve III"],
        answer: "I, II ve III"
    },

    {
        number: 41,
        question: "Fotoğraf makinesinin ana yapısını oluşturan iki mekanizma aşağıdakilerden hangisinde doğru verilmiştir?",
        options: ["Vizör-LCD ekran", "Objektif-gövde", "Batarya-Flaş", "Hafıza kartı-flaş", "Diyafram-zoom halkası"],
        answer: "Objektif-gövde"
    },
    {
        number: 42,
        question: "Objektif üzerinde bulunan mekanizmalar arasında aşağıdakilerden hangisi yoktur?",
        options: ["Netleme halkası", "Vibrasyon değeri", "Diyafram değerleri", "Mod kadranı", "Zoom halkası"],
        answer: "Mod kadranı"
    },
    {
        number: 43,
        question: "Fotoğraf makinesinde diyafram değerini gösteren ifade aşağıdakilerden hangisidir?",
        options: ["f numarası", "z numarası", "m numarası", "d numarası", "k numarası"],
        answer: "f numarası"
    },
    {
        number: 44,
        question: "Fotoğrafta konuların ayrıntılarının miktarını belirleyen netlik, fotoğraf makinesinde aşağıdaki hangi halka üzerinden yapılır?",
        options: ["Zoom halkası", "Focus halkası", "Diyafram halkası", "Objektif halkası", "Mod halkası"],
        answer: "Focus halkası"
    },
    {
        number: 45,
        question: "Fotoğraf makinesinin tüm sistem ve işlevlerini gösteren ekran olarak bilgi panosu, vizör, bilgisayar monitörü ve playback yerine aşağıdakilerden hangisi geçmektedir?",
        options: ["Flaş", "Info", "LCD ekran", "Ön izleme düğmesi", "Diyoptri düğmesi"],
        answer: "LCD ekran"
    },
    {
        number: 46,
        question: "Fotoğraf makinesinin gövdesinde…………………, …………………… ve ………….. düğmeleri bulunur. Cümlede boş bırakılan yerlere sırasıyla yukarıdakilerden hangileri getirilmelidir?",
        options: ["I, II  ve III", "I, III ve IV", "II, III ve IV", "II, IV ve V", "III, IV ve V"],
        answer: "I, II  ve III"
    },
    {
        number: 47,
        question: "Fotoğraf makinesinde bulunan deklanşörün tanımı aşağıdakilerden hangisidir?",
        options: ["Netlik sağlayan düğmedir.", "Diyafram ayarlarını yapan düğmedir.", "Fotoğraf çekilirken basılan düğmedir.", "Işığın yetersiz olduğu durumlarda kullanılan düğmedir.", "Ön izleme yapabilmek için basılan düğmedir."],
        answer: "Fotoğraf çekilirken basılan düğmedir."
    },
    {
        number: 48,
        question: "Fotoğraf makinelerine takılan hafıza kartı çeşitleri arasında aşağıdakilerden hangisi bulunmaz?",
        options: ["Secure Digital (SD) kart", "Multimedia (MMC) kart", "Smart Media (SM) kart", "Compact Flash (CF) kart", "Compact Disk (CD) kart"],
        answer: "Compact Disk (CD) kart"
    },
    {
        number: 49,
        question: "Fotoğraf makinesinden depolama alanına veri aktarımı yapan çıkışlardan biri aşağıdakilerden hangisidir?",
        options: ["USB çıkışı", "Aux çıkışı", "VGA çıkışı", "Kompozit çıkışı", "A/V çıkışı"],
        answer: "USB çıkışı"
    },
    {
        number: 50,
        question: "Göz ile vizörün buluştuğu noktada netliğin ayarlanmasını sağlayan, fotoğrafın net olması için kullanılan düğme aşağıdakilerden hangisidir?",
        options: ["Info", "Diyoptri", "Vibrasyon", "Menü", "Lens çıkarma"],
        answer: "Diyoptri"
    },

    {
        number: 51,
        question: "Bir fotoğrafın aydınlık ve karanlık oluşu anlamına aşağıdakilerden hangisi gelmektedir?",
        options: ["Diyafram", "Odak uzaklığı", "Enstantane", "Pozlama", "ISO"],
        answer: "Pozlama"
    },
    {
        number: 52,
        question: "Diyafram tarafından ışığın objektiften geçmesine izin veren delik ya da açıklığa ne ad verilmektedir?",
        options: ["ISO", "Diyafram boyutu", "Enstantane", "İris diyafram", "Diyafram açıklığı"],
        answer: "Diyafram açıklığı"
    },
    {
        number: 53,
        question: "Filmin ışığa duyarlılık hassasiyetini belirleyen unsur aşağıdakilerden hangisidir?",
        options: ["F/stop", "Alan derinliği", "Basit diyafram", "ISO", "Enstantane"],
        answer: "ISO"
    },
    {
        number: 54,
        question: "Görüntüsü alınan objeden yansıyan ışınların aynanın üzerine düşeceği yoğunluğu belirleyen sisteme ne ad verilmektedir?",
        options: ["ISO", "F/durak", "Diyafram", "Enstantane", "Işık yoğunluğu"],
        answer: "Diyafram"
    },
    {
        number: 55,
        question: "Diyafram, deklanşör hızı ve ISO arasındaki dengeleme eylemi, fotoğrafta ne olarak adlandırılmaktadır?",
        options: ["Enstantane", "Demir üçgen", "F/durak", "Basit diyafram", "Açık diyafram"],
        answer: "Demir üçgen"
    },
    {
        number: 56,
        question: "Bir fotoğrafta net odakta görünen en yakın ve en uzak nesneler arasındaki mesafe miktarına ne ad verilmektedir?",
        options: ["Diyafram boyutu", "Diyafram açıklığı", "Alan derinliği", "Işık yoğunluğu", "Pozlama"],
        answer: "Alan derinliği"
    },
    {
        number: 57,
        question: "Diyafram açıklıklarının aydınlanma koşullarına göre otomatik olarak sağlanması için yapılmış diyafram sistemi aşağıdakilerden hangisidir?",
        options: ["Basit diyafram", "İris diyafram", "Odak diyafram", "Otomatik diyafram", "Diyafram açıklığı"],
        answer: "Otomatik diyafram"
    },
    {
        number: 58,
        question: "Bir merceğin bir görüntüyü veya sahneyi ne kadar büyüttüğünü tanımlayan kavram aşağıdakilerden hangisidir?",
        options: ["Odak uzaklığı", "Odak noktası", "Işık yoğunluğu", "Alan derinliği", "Diyafram boyutu"],
        answer: "Odak uzaklığı"
    },
    {
        number: 59,
        question: "Daha az alan derinliği veya sığ alan derinliği anlamında aşağıdakilerden hangisi kullanılmaktadır?",
        options: ["F/stop", "Düşük f/stop sayısı", "Yüksek f/stop sayısı", "f/durak", "Enstantane"],
        answer: "Düşük f/stop sayısı"
    },
    {
        number: 60,
        question: "Bir nesnenin görüntüsünün istenilen biçimde çekilmesi için cisimlerin üzerine düşen ya da etrafındaki ışık miktarını ayarlamak için enstantane ayarına uygun yapılması gereken ayar aşağıdakilerden hangisidir?",
        options: ["F/durak", "Alan derinliği", "Odak noktası", "Diyafram ayarı", "ISO"],
        answer: "Diyafram ayarı"
    },

    {
        number: 61,
        question: "Hareketli bir nesnenin hareketi flu bir şekilde aşağıdaki değerlerden hangisinde fotoğraflanabilir?",
        options: ["1/2000", "1/2", "1/4000", "1/8000", "1/1000"],
        answer: "1/2"
    },
    {
        number: 62,
        question: "Enstantane değerleri aşağıdakilerden hangisinin hareketiyle elde edilir?",
        options: ["Vizör", "Sensör", "Film", "Obtüratör", "Pixell"],
        answer: "Obtüratör"
    },
    {
        number: 63,
        question: "Yüksek ASA’dan kaynaklanan görüntü bozulması ne olarak adlandırılır?",
        options: ["Işık", "Gümüş nitrat", "Grain", "Ayna", "Perde"],
        answer: "Grain"
    },
    {
        number: 64,
        question: "Normal değer olarak kabul edilen değer aşağıdaki enstantane değerlerinden hangisidir?",
        options: ["1/125", "1/500", "1/2000", "1/30", "1/250"],
        answer: "1/125"
    },
    {
        number: 65,
        question: "Enstantane değerlerinden hangisinde ışık 1/250 değerinden 3 kat daha hızlı sürede sensöre ulaşır?",
        options: ["1/15", "1/60", "1/4000", "1/1000", "1/2000"],
        answer: "1/2000"
    },
    {
        number: 66,
        question: "Noise oranı aşağıdaki değerlerden hangisinde en yüksektir?",
        options: ["ISO 100", "ISO 3200", "ISO 400", "ISO 1600", "ISO 800"],
        answer: "ISO 3200"
    },
    {
        number: 67,
        question: "Dijital fotoğraf makinelerinde yüksek ISO’dan kaynaklanan görüntü bozulması ne olarak adlandırılır?",
        options: ["Piksel", "Ayna", "Noise", "Obtüratör", "Perde"],
        answer: "Noise"
    },
    {
        number: 68,
        question: "ASA değerleri aşağıdakilerden hangisine bağlı olarak değişiklik gösterir?",
        options: ["Gümüş zerrecikleri", "Piksel", "Sensör", "Nesnenin hareketi", "Obtüratör"],
        answer: "Gümüş zerrecikleri"
    },
    {
        number: 69,
        question: "Eski doğu bloku ülkelerinde ASA yerine kullanılan kavram aşağıdakilerden hangisidir?",
        options: ["ISO", "Grain", "Stop", "DIN", "Gümüş nitrat"],
        answer: "DIN"
    },
    {
        number: 70,
        question: "ISO 100 ve 1/60 enstantane değerinde elde edilen poz değerinin aynısını 1/2000 enstantane değerinde elde edebilmek için ISO değeri kaç olmalıdır?",
        options: ["6400", "800", "400", "50", "3200"],
        answer: "3200"
    },

    {
        number: 71,
        question: "Fotoğrafınızın ışık değerinin gerekenden az olması durumunda yapılması gereken yardımcı pozlama ayarı aşağıdakilerden hangisidir?",
        options: ["WB ayarı", "Flaş ayarı", "Pozlama değeri (EV) +’ya doğru ayarı", "Uv Filtre ayarı", "Noktasal ölçüm modu ayarı"],
        answer: "Pozlama değeri (EV) +’ya doğru ayarı"
    },
    {
        number: 72,
        question: "Düz ve temiz bir fon önünde ters ışıkla oluşacak siluetler ile yakalayacağınız kontrast biçimi aşağıdakilerden hangisidir?",
        options: ["Leke kontrastı", "İçerik kontrastı", "Nokta kontrastı", "Çizgisel kontrast", "Şekilsel kontrast"],
        answer: "Leke kontrastı"
    },
    {
        number: 73,
        question: "Fotoğrafındaki karanlık bölgeleri arındırarak bir kontrast sağlamak istediğinizde makinenizdeki hangi ayar ile oynamanız gerekmektedir?",
        options: ["Whites- Blacks", "Highlights", "White balance", "Image size", "Shadows"],
        answer: "Shadows"
    },
    {
        number: 74,
        question: "Kadrajdaki birçok noktadan farklı ışık yoğunluklarını görüp ve onların ortalamasını alarak alan bilgisi ile birlikte bir hesaplama yapan ölçüm yöntemi aşağıdakilerden hangisidir?",
        options: ["Merkez ağırlıklı", "Noktasal", "Genel", "Kısmi", "Çok bölgeli"],
        answer: "Çok bölgeli"
    },
    {
        number: 75,
        question: "Milyonlarca yatay ve dikey görüntünün birleşmesi ile oluşan sayısal görüntüleri, fotoğrafı hangi çözünürlükte ve kalitede çekebileceğinizi ifade eden kavram aşağıdakilerden hangisidir?",
        options: ["Iso değeri", "Pixel", "Vizör", "Diyafram", "Enstantane"],
        answer: "Pixel"
    },
    {
        number: 76,
        question: "Çekim esnasındaki ham görüntünün herhangi bir işleme maruz kalmadan doğrudan kaydedilmesine olanak sağlayan özel format aşağıdakilerden hangisidir?",
        options: ["JPEG", "TIFF", "BMP", "RAW", "GIF"],
        answer: "RAW"
    },
    {
        number: 77,
        question: "Beklenenden daha az netliğe sahip bir fotoğrafa netlik kazandırmak amacıyla yapılan ayar aşağıdakilerden hangisidir?",
        options: ["Kontrast", "Image sharpening", "UV Filtre", "ISO", "WB"],
        answer: "Image sharpening"
    },
    {
        number: 78,
        question: "Renk matriksi veya değerlendirmeli ölçüm olarak da bilinen ve fotoğraf makinesinde simgesi ile verilen ölçüm yöntemi aşağıdakilerden hangisidir?",
        options: ["Noktasal", "Merkez ağırlıklı", "Çok bölgeli", "Genel", "Kısmi"],
        answer: "Genel"
    },
    {
        number: 79,
        question: "6000 Kelvin’e ayarlanmış beyaz ayarı değerinde olan ve gün ışığı ve gölge arasında tonlara sahip olan beyaz ayarı yaparken hangi modu seçersiniz?",
        options: ["Gün ışığı (Daylight)", "Gölge (Shade)", "Bulutlu (Cloudy)", "Ampül (Tungsten)", "Floresan (Fluorescent)"],
        answer: "Bulutlu (Cloudy)"
    },
    {
        number: 80,
        question: "Beyaz ayarında simgesi ile seçeceğiniz mod aşağıdakilerden hangisidir?",
        options: ["Seçimli / Özel (Custom)", "Kelvin", "Gölge (Shade)", "Ampül (Tungsten)", "Otomatik (Auto)"],
        answer: "Otomatik (Auto)"
    },

    {
        number: 81,
        question: "Objektifin işlevleri ve özellikleri arasında aşağıdakilerden hangisi yer almaz?",
        options: ["Konudan yansıyan ışığın kamera içindeki yüzeye düşmesini sağlar.", "Konudan yansıyan ışınları belli bir noktaya odaklar.", "Nesnenin görüntüsünü oluşturan parçadır.", "Sadece kalın kenarlı merceklerden oluşur.", "Objektifin görüş açısı odak uzaklığı ile ters orantılıdır."],
        answer: "Sadece kalın kenarlı merceklerden oluşur."
    },
    {
        number: 82,
        question: "Odak uzaklığı ile ilgili aşağıdakilerden hangisi söylenemez?",
        options: ["Odak uzaklığı milimetre olarak ifade edilir.", "Odak uzaklığı arttıkça merceğin yakınlaşması azalır.", "Uzun odak uzunluğunda nesneler daha yakındaymış gibi görünür.", "İnsan gözünün odak uzaklığı ortalama 50 mm’dir.", "Odak uzaklığı ile görüş alanı ters orantılıdır."],
        answer: "Odak uzaklığı arttıkça merceğin yakınlaşması azalır."
    },
    {
        number: 83,
        question: "Nesnelerden gelen ışınların duyarkatta toplanma gücünün ölçüsü aşağıdaki kavramların hangisiyle ifade edilir?",
        options: ["Diyafram", "Görüş alanı", "Objektif hızı", "Mercek", "Odak uzaklığı"],
        answer: "Objektif hızı"
    },
    {
        number: 84,
        question: "Görüntüde muhtemel renk sapmalarını engellemek için alınabilecek tedbirler arasında aşağıdakilerden hangisi yoktur?",
        options: ["Tripod kullanmamak", "Çok keskin renk karşıtlıkları içeren sahnelerden kaçınmak", "Diyafram açıklığını ayarlamak", "Kaliteli objektif tercih etmek", "Çekim amacına uygun odak uzaklığı seçmek"],
        answer: "Tripod kullanmamak"
    },
    {
        number: 85,
        question: "Fotoğrafın bulanık çıkmasının nedenleri arasında aşağıdakilerden hangisi yoktur?",
        options: ["Konunun hareketli olması", "Pozlama süresinin fazla olması", "Konunun karşıt renkler içermesi", "Ortamda yeterli ışık bulunmaması", "Çekim esnasında fotoğraf makinesinin hareket ettirilmesi"],
        answer: "Konunun karşıt renkler içermesi"
    },
    {
        number: 86,
        question: "Objektifin bileşenleri arasında aşağıdakilerden hangisi yer almaz?",
        options: ["Bayonet", "Netleme halkası", "Objektif çerçevesi", "Diyafram perdesi", "Vizör"],
        answer: "Vizör"
    },
    {
        number: 87,
        question: "Geniş açılı objektiflerin özellikleri arasında aşağıdakilerden hangisi yer almaz?",
        options: ["Uzun odak uzaklıklı objektiflerdir.", "Alan derinlikleri fazladır.", "Odak uzaklıkları 50 mm’den azdır.", "Manzara veya kalabalık sahne çekimleri için idealdir.", "İnsan gözünün gördüğü açıdan daha geniş bir alanı yakalayabilirler."],
        answer: "Uzun odak uzaklıklı objektiflerdir."
    },
    {
        number: 88,
        question: "Değişken odak uzaklıklı objektifleri aşağıdakilerden hangisi tanımlamaz?",
        options: ["Farklı odak uzaklığına sahip objektiflerin özelliklerini bir arada barındıran objektiflerdir.", "Odak uzaklığı değişebilen objektiflerdir.", "Hem geniş hem de dar açılı görüntülerin yakalanmasına imkân tanıyan objektiflerdir.", "Standart objektiflere göre gövdesinde daha fazla merceğin bulunduğu objektiflerdir.", "Görüş açısı en geniş objektiflerdir."],
        answer: "Görüş açısı en geniş objektiflerdir."
    },
    {
        number: 89,
        question: "Yansımayı azaltarak parlaklığı önleyen ve renklerin daha canlı görünmesini sağlayan filtre hangisidir?",
        options: ["Vidalı filtre", "Nötr yoğunluklu filtre", "Gri-siyah renk filtresi", "Polarize filtre", "UV filtre"],
        answer: "Polarize filtre"
    },
    {
        number: 90,
        question: "ND filtrelerin özellikleri arasında aşağıdakilerden hangisi yer almaz?",
        options: ["Dereceli filtre olarak da adlandırılır.", "Objektife gelen ışığın miktarını arttırır.", "Gri-siyah renkte üretilir.", "Özellikle doğa fotoğrafçılığında kullanılır.", "Filtrenin koyuluk derecesi arttıkça parlaklığı düşürme oranı da artar."],
        answer: "Objektife gelen ışığın miktarını arttırır."
    },

    {
        number: 91,
        question: "Konuya göre objektif seçiminde etkili olan temel nedenler arasında aşağıdakilerden hangisi bulunmaz?",
        options: [
            "Konunun genişliği",
            "Konunun uzaklığı",
            "Konunun büyüklüğü",
            "Kompozisyon",
            "Konunun içerdiği renkler"
        ],
        answer: "Konunun içerdiği renkler"
    },
    {
        number: 92,
        question: "Geniş açılı objektiflerin özellikleri arasında aşağıdakilerden hangisi yer almaz?",
        options: [
            "Dar açılı objektiflere göre daha fazla alan kadraja alınır.",
            "Nesneler olduğundan daha uzaktaymış gibi görünür.",
            "Nesnenin büyütülme oranı, dar açılı objektiflere göre daha fazladır.",
            "Görüş alanı, dar açılı objektiflere göre daha fazladır.",
            "Geniş açılı objektiflerin verdiği derinlik hissi, dar açılı objektiflere göre daha fazladır."
        ],
        answer: "Nesnenin büyütülme oranı, dar açılı objektiflere göre daha fazladır."
    },
    {
        number: 93,
        question: "Portre fotoğrafçılığında kullanılan bir objektifin ideal odak uzaklığını aşağıdakilerden hangisi belirtir?",
        options: [
            "7-15 mm",
            "45-55 mm",
            "24-70 mm",
            "85-100 mm",
            "100-300 mm"
        ],
        answer: "85-100 mm"
    },
    {
        number: 94,
        question: "Portre fotoğrafçılığında odak noktası genellikle neresidir?",
        options: [
            "Gözler",
            "Çevre",
            "Beden",
            "Arka plan",
            "Omuzlar"
        ],
        answer: "Gözler"
    },
    {
        number: 95,
        question: "Manzara fotoğrafı çekerken aşağıdakilerden hangisi dar açılı objektif kullanmayı gerektiren bir durumdur?",
        options: [
            "Geniş bir görüş açısı elde etmek",
            "Konunun belli bir noktasına yakınlaşmak",
            "Geniş alan derinliği sağlamak",
            "Yansımaları azaltmak",
            "Kısa odak uzaklığı sayesinde geniş bir alanı tek bir kareye sığdırmak"
        ],
        answer: "Konunun belli bir noktasına yakınlaşmak"
    },
    {
        number: 96,
        question: "Vahşi yaşam fotoğrafçılığında teleobjektif kullanılmasının nedenleri arasında aşağıdakilerden hangisi bulunmaz?",
        options: [
            "Geniş görüş açısı sağlanarak çevrenin kadraja dâhil edilmesi",
            "Konuya bağlı olarak can güvenliğini tehdit edecek muhtemel tehlikeler olması",
            "Konunun doğal hareketlerine etki etmeyecek kadar mesafenin korunması",
            "Konuyu ürkütmeden fotoğraflama imkânı sağlaması",
            "Coğrafi şartların elverişli olmadığı durumlarda konuya yaklaşmadan görüntü elde edilmesi"
        ],
        answer: "Geniş görüş açısı sağlanarak çevrenin kadraja dâhil edilmesi"
    },
    {
        number: 97,
        question: "Dış mimari fotoğrafçılığında objektif seçimini belirleyen faktörler arasında aşağıdakilerden hangisi yer almaz?",
        options: [
            "Yapıyla fotoğrafçı arasındaki mesafe",
            "Yapının başka binalarla çevrelenmiş olması",
            "Gün doğumu veya gün batımı gibi güneş ışınlarının geniş açıyla geldiği saatlerde çekim yapılması",
            "Yapının kapladığı alan",
            "Yapının bağlamıyla birlikte verilmek istenmesi"
        ],
        answer: "Gün doğumu veya gün batımı gibi güneş ışınlarının geniş açıyla geldiği saatlerde çekim yapılması"
    },
    {
        number: 98,
        question: "İç mekân çekimlerinde geniş açılı objektif kullanılmasının nedenleri arasında aşağıdakilerden hangisi bulunmaz?",
        options: [
            "Fotoğrafta sıkışıklık hissine engel olmak",
            "Geniş alan derinliği elde etmek",
            "Mekânı ferah göstermek",
            "Mümkün olan en geniş görüş açısıyla mekânı bütünlüklü fotoğraflamak",
            "Görüntüyü bozmadan çerçeveyi dar tutmak"
        ],
        answer: "Görüntüyü bozmadan çerçeveyi dar tutmak"
    },
    {
        number: 99,
        question: "İç mimari fotoğrafçılığı olarak aşağıdakilerden hangisi değerlendirilemez?",
        options: [
            "Tiyatro sahnesi",
            "Cami minaresi",
            "Hastane koridorları",
            "Bir ofisin oturma planı",
            "Oturma alnında yer alan dekoratif objeler"
        ],
        answer: "Cami minaresi"
    },
    {
        number: 100,
        question: "Çok yakın çekim yaparken makro objektifin sağladığı avantajlar arasında aşağıdakilerden hangisi yer almaz?",
        options: [
            "Konuya minimum düzeyde yakınlaşılması",
            "Çok yakın mesafeden netlik sağlanması",
            "Konunun kendi boyutunda veya büyütülerek kaydedilmesi",
            "Çok geniş bir alan derinliği yaratılması",
            "Konuya yakın mesafeden çekim yapılarak kadrajın konuyla doldurulması"
        ],
        answer: "Çok geniş bir alan derinliği yaratılması"
    },

    {
        number: 101,
        question: "Panoramik fotoğraf türünün özellikleri arasında aşağıdakilerden hangisi bulunmaz?",
        options: [
            "Geniş açılı fotoğraflardır",
            "Soldan sağa 180 dereceyi kaplayan panoramalardır",
            "360 dereceye kadar olan panoramalardır",
            "Küçük detaylara odaklanılır",
            "Birden fazla görüntüden oluşabilmektedir"
        ],
        answer: "Küçük detaylara odaklanılır"
    },
    {
        number: 102,
        question: "Aynı zamanda bir fotoğraf derneği olan AFSAD tarafından aşağıdaki fotoğraf dergilerinden hangisi yayımlanmaktadır?",
        options: [
            "Kontrast",
            "National Geographic",
            "Atlas",
            "Fotoğraf",
            "PhotoWorld"
        ],
        answer: "Kontrast"
    },
    {
        number: 103,
        question: "Daha çok spor aktivitelerinde, örneğin futbol maçlarında önemli anların tekrar ağır çekimde verilmesi, doğa olaylarında, dizi ve filmlerde kullanılan fotoğraf çekim tekniği aşağıdakilerden hangisidir?",
        options: [
            "Time Lapse",
            "Hyper Lapse",
            "Küratör",
            "Slow Motion",
            "Flickr"
        ],
        answer: "Slow Motion"
    },
    {
        number: 104,
        question: "Fotoğraf sanatı ya da fotoğrafçılar açısından, bireysel etkinliklerin sergilenmesi, kişilerin fotoğraflarının tanıtılması anlamına gelen kavram aşağıdakilerden hangisidir?",
        options: [
            "Photobucket",
            "Panaroma",
            "Portfolyo",
            "Küratör",
            "Hyper Lapse"
        ],
        answer: "Portfolyo"
    },
    {
        number: 105,
        question: "Yeni medyada fotoğraf paylaşım platformları arasında aşağıdakilerden hangisi bulunmaz?",
        options: [
            "Flickr",
            "Picasa",
            "Instagaram",
            "Photobucket",
            "PhotoWorld"
        ],
        answer: "PhotoWorld"
    },
    {
        number: 106,
        question: "Hyperlapse fotoğraf tekniğinin özellikleri arasında aşağıdakilerden hangisi bulunmaz?",
        options: [
            "Her pozlamadan sonra kameranın konumu değişir.",
            "Çok sayıda fotoğraf ya da videodan oluşur.",
            "Bir Instagram uygulaması olarak da kullanılmaktadır.",
            "Yüksek kaliteli zaman aralıklı videoların birleşmesiyle oluşur.",
            "Görüntülerin yavaşlatılması esasına dayanır."
        ],
        answer: "Görüntülerin yavaşlatılması esasına dayanır."
    },
    {
        number: 107,
        question: "Fotoğraf üzerindeki düşük, orta ve yüksek aralıktaki ışığın ortalamasını alarak bunların uygun bir şekilde yüzeye dağılımını sağlayan teknik aşağıdakilerden hangisidir?",
        options: [
            "Picasa",
            "Time Lapse",
            "HDR",
            "3D",
            "Stop motion"
        ],
        answer: "HDR"
    },
    {
        number: 108,
        question: "Bilim, tarih, coğrafya, kültür gibi alanlardaki makaleler ve yayınladığı kaliteli fotoğraflarla dünyaca bilinen ve 37 dilde yayımlanan dergi aşağıdakilerden hangisidir?",
        options: [
            "AFSAD",
            "Kontrast",
            "Fotoğraf",
            "National Geographic",
            "Atlas"
        ],
        answer: "National Geographic"
    },
    {
        number: 109,
        question: "Ortamdaki aşırı aydınlık ve aşırı karanlık bölgelerin ortalama değerlere göre ayarlandığı fotoğraf çekim tekniği aşağıdakilerden hangisidir?",
        options: [
            "Timelapse",
            "Hyper Lapse",
            "Küratör",
            "Slow Motion",
            "HDR"
        ],
        answer: "HDR"
    },
    {
        number: 110,
        question: "Ülkemizde web üzerinden yayın yapan, 1995 yılında kurulan ve fotoğraf sanatı ile ilgili her türlü bilginin bulunduğu dergi aşağıdakilerden hangisidir?",
        options: [
            "Atlas",
            "Kontrast",
            "FotoNo1",
            "National Geography",
            "Fotoğraf"
        ],
        answer: "Fotoğraf"
    },
    
    {
        number: 111,
        question: "Fotoğrafta kadrajı oluşturan biçimsel ögeler arasında aşağıdakilerden hangisi bulunmaz?",
        options: [
            "Işık",
            "Renk",
            "Perspektif",
            "Anlam",
            "Çizgi"
        ],
        answer: "Anlam"
    },
    {
        number: 112,
        question: "Fotoğrafta bir hareketlilik oluşturarak dinamik bir etki katan çizgi türü aşağıdakilerden hangisidir?",
        options: [
            "Çapraz çizgiler",
            "Yatay çizgiler",
            "Dikey çizgiler",
            "Yuvarlak çizgiler",
            "Eğri çizgiler"
        ],
        answer: "Çapraz çizgiler"
    },
    {
        number: 113,
        question: "Fotoğrafçılıkta iki boyutlu, derinliği olmayan bir nesnenin taslağı olan kapalı alanları ifade eden kavram aşağıdakilerden hangisidir?",
        options: [
            "Form",
            "Biçim",
            "Renk",
            "Işık",
            "Şekil"
        ],
        answer: "Şekil"
    },
    {
        number: 114,
        question: "Bir şeklin; en, boy ve yükseklikten oluşan üç boyutlu halini ifade eden kavram aşağıdakilerden hangisidir?",
        options: [
            "Çizgi",
            "Form",
            "Renk",
            "Işık",
            "Şekil"
        ],
        answer: "Form"
    },
    {
        number: 115,
        question: "I. Bakış açısı, \nII.objektifin odak uzaklığı \nIII. diyafram değerlerir, \nYukarıdakilerden hangisi ya da hangileri bir fotoğrafta üç boyut hissi oluşturmak amacıyla kullanılmaktadır?",
        options: [
            "Yalnız I",
            "Yalnız II",
            "Yalnız III",
            "I ve II",
            "I, II ve III"
        ],
        answer: "I, II ve III"
    },
    {
        number: 116,
        question: "Renklerden hangisi soğuk bir renktir?",
        options: [
            "Kırmızı",
            "Koyu kırmızı",
            "Sarı",
            "Mavi",
            "Turuncu"
        ],
        answer: "Mavi"
    },
    {
        number: 117,
        question: "Sabah güneş doğduktan sonra ve akşam güneş batmadan önceki ortalama bir saatlik zaman dilimini ifade eden terim aşağıdakilerden hangisidir?",
        options: [
            "Mavi saatler",
            "Gri saatler",
            "Altın saatler",
            "Gümüş saatler",
            "Parlak saatler"
        ],
        answer: "Altın saatler"
    },
    {
        number: 118,
        question: "Işık kaynağının konunun arkasında ve dolayısıyla fotoğrafçının önünde olduğu ışık kullanım biçimi aşağıdakilerden hangisidir?",
        options: [
            "Yanal ışık",
            "Tepe aydınlatma",
            "Dolgu ışık",
            "Ters ışık",
            "Cephe ışığı"
        ],
        answer: "Ters ışık"
    },
    {
        number: 119,
        question: "Bir yüzeyi oluşturan şeklin, maddenin veya malzemenin yapısının yüzey bilgisi aşağıdakilerden hangisidir?",
        options: [
            "Dolgu",
            "Doku",
            "Derinlik",
            "Perspektif",
            "Renk"
        ],
        answer: "Doku"
    },
    {
        number: 120,
        question: "Bir fotoğrafın ön plandaki nesne ile arka plandaki nesne arasında hiçbir uzaklık yokmuş gibi görünmesini ifade eden kavram aşağıdakilerden hangisidir?",
        options: [
            "Üç boyut hissi",
            "Derinlik algısı",
            "Hacim duygusu",
            "Alan derinliği",
            "Perspektif yığılması"
        ],
        answer: "Perspektif yığılması"
    },

    {
        number: 121,
        question: "Fotoğrafta kompozisyon uygulamalarının içsel ögeleri arasında aşağıdakilerden hangisi bulunmaz?",
        options: [
            "Fon kullanımı",
            "Şekiller",
            "Üçte bir kuralı",
            "Ilgi merkezi",
            "Duygusal vurgu"
        ],
        answer: "Şekiller"
    },
    {
        number: 122,
        question: "Fotoğrafı çekilecek bir konunun görsel açıdan düzenlenmesini ifade eden kavram aşağıdakilerden hangisidir?",
        options: [
            "Kadraj",
            "Çerçeveleme",
            "Kompozisyon",
            "Grafik etkisi",
            "Kartpostal etkisi"
        ],
        answer: "Kompozisyon"
    },
    {
        number: 123,
        question: "Bir görüntünün ilgi merkezinin, fotoğraf karesinin yatay ve dikey üç eşit parçaya bölünmesi sonucunda oluşan kesişme noktalarına yerleştirmesi kuralı aşağıdakilerden hangisidir?",
        options: [
            "Üçte bir kuralı",
            "Arka plan kuralı",
            "Duygusal vurgu",
            "Fon kullanımı",
            "Arka plan kullanımı"
        ],
        answer: "Üçte bir kuralı"
    },
    {
        number: 124,
        question: "I: Mekânda çok sayıda ve dağınık nesne varsa \nII: egemen öge kadrajı dolduruyorsa \nIII. ilgi ön plandaki konuya çekilmek isteniyorsa \nYukarıdakilerden hangisi ya da hangilerinde fonun bulanık (flu) bırakılması tercih edilmektedir?",
        options: [
            "Yalnız I",
            "Yalnız II",
            "Yalnız III",
            "I ve II",
            "I, II ve III"
        ],
        answer: "I, II ve III"
    },
    {
        number: 125,
        question: "Bir fotoğrafta bakışların yoğunlaşmasında ve anlamın oluşmasında merkezde bulunan temel içsel öge aşağıdakilerden hangisidir?",
        options: [
            "Alan derinliği",
            "Flu",
            "Perspektif",
            "Ilgi merkezi",
            "Perspektif"
        ],
        answer: "Ilgi merkezi"
    },
    {
        number: 126,
        question: "Kompozisyonu oluşturan bütün ögelerin tamamının kadrajın içerisinde yer aldığı kompozisyon türü aşağıdakilerden hangisidir?",
        options: [
            "Açık kompozisyon",
            "Sonsuz kompozisyon",
            "Kapalı kompozisyon",
            "Yinelemeli kompozisyon",
            "Ritmik kompozisyon"
        ],
        answer: "Kapalı kompozisyon"
    },
    {
        number: 127,
        question: "Çok sayıda konunun yer aldığı, derinlikli bir fotoğrafta kaç plan bulunmaktadır?",
        options: [
            "Bir",
            "İki",
            "Üç",
            "Dört",
            "Sonsuz"
        ],
        answer: "Üç"
    },
    {
        number: 128,
        question: "İnsanda var olduğu kabul edilen temel duygular arasında aşağıdakilerden hangisi bulunmaz?",
        options: [
            "Korku",
            "Öfke",
            "Mutluluk",
            "Üzüntü",
            "Şaşkınlık"
        ],
        answer: "Şaşkınlık"
    },
    {
        number: 129,
        question: "Fotoğrafın gücü ve etkileri açısından aşağıdakilerden hangisi diğerlerinden önce gelmektedir?",
        options: [
            "Duyguları harekete geçirme özelliği",
            "Bilgi verme potansiyeli",
            "Mesaj içerme yönü",
            "Kanıt olma özelliği",
            "Eğitme özelliği"
        ],
        answer: "Duyguları harekete geçirme özelliği"
    },
    {
        number: 130,
        question: "I. Çok sayıda öge kullanma \nII. kadrajda kapladığı alanı büyütme \nIII. üçte bir kuralına göre yerleştirme \nYukarıdakilerden hangisi ya da hangileri fotoğraftaki egemen ögeyi ilgi merkezine dönüştürebilmek amacıyla kullanılan yöntemler arasında yer alır?",
        options: [
            "Yalnız I",
            "Yalnız II",
            "I ve II",
            "II ve III",
            "I, II ve III"
        ],
        answer: "II ve III"
    },

    {
        number: 131,
        question: "Fotoğrafta kompozisyonu etkili hale getiren anlatım biçimleri arasında aşağıdakilerden hangisi bulunmaz?",
        options: [
            "Boşluk",
            "Belirginlik",
            "Sadelik",
            "Hareket",
            "Bakış yönü"
        ],
        answer: "Boşluk"
    },
    {
        number: 132,
        question: "Fotoğrafta belirginliği arttıran unsurlar arasında aşağıdakilerden hangisi bulunmaz?",
        options: [
            "Çizgi",
            "Ritim",
            "Renk",
            "Doku",
            "Biçim"
        ],
        answer: "Ritim"
    },
    {
        number: 133,
        question: "“Fotoğrafta kadraj içerisine daha az nesne dahil ederek daha etkili bir anlatım biçimi elde etmektir.” ifadesi ile aşağıdakilerden hangisi tanımlanmaktadır?",
        options: [
            "Boşluk",
            "Form",
            "Sadelik",
            "Işık",
            "Biçim"
        ],
        answer: "Sadelik"
    },
    {
        number: 134,
        question: "Başarılı ve amacına uygun bir fotoğrafik kompozisyon elde etmek aşağıdakilerden hangisine bağlı değildir?",
        options: [
            "ISO değeri",
            "İletişim",
            "Işık",
            "Uygun an",
            "Renk kullanımı"
        ],
        answer: "ISO değeri"
    },
    {
        number: 135,
        question: "“Görsel bir anlatım dili olan fotoğrafta anlatılmak istenen konunun ya da mesajın kolay bir şekilde algılanmasıdır.” ifadesi ile aşağıdaki hangi anlatım biçimi tanımlanmaktadır?",
        options: [
            "1/3 kuralı",
            "Altın kesim",
            "Doğal çerçeve",
            "Belirginlik",
            "Kadraj"
        ],
        answer: "Belirginlik"
    },
    {
        number: 136,
        question: "Fotoğrafta sade bir anlatım dili oluşturmanın yolları arasında aşağıdakilerden hangisi bulunmaz?",
        options: [
            "Geniş açılı objektif kullanmak",
            "Dar açılı objektif kullanmak",
            "Konuya yaklaşarak çekim yapmak",
            "Temiz bir arka plan kullanmak",
            "Geniş olmayan diyafram açıklığı kullanmak"
        ],
        answer: "Geniş açılı objektif kullanmak"
    },
    {
        number: 137,
        question: "I. Kişisel bir seçimdir.\nII. Bir karar almadır.\nIII. Birleştirme sürecidir.\nYukarıdakilerden hangisi ya da hangileri fotoğrafın özellikleri arasında yer alır?",
        options: [
            "Yalnız I",
            "Yalnız II",
            "Yalnız III",
            "I ve II",
            "I, II ve III"
        ],
        answer: "I, II ve III"
    },
    {
        number: 138,
        question: "Bir yansıma ya da simetri fotoğrafı çekebilmek için gerekli ögeler arasında aşağıdakilerden hangisi bulunmaz?",
        options: [
            "Su birikintileri",
            "Göller",
            "Islak zeminler",
            "Ayna gibi yansıma yapan yüzeyler",
            "Göz seviyesinden çekim yapmak"
        ],
        answer: "Göz seviyesinden çekim yapmak"
    },
    {
        number: 139,
        question: "Fotoğrafta doğal bir çerçeve oluşturmak için aşağıdakilerden hangisi kullanılamaz?",
        options: [
            "Ağaç dalları",
            "Gökyüzü",
            "Kemerli yapılar",
            "Pencereler",
            "Ağaçların gövde kısımları"
        ],
        answer: "Gökyüzü"
    },
    {
        number: 140,
        question: "Fotoğrafta sanal olarak iki yatay ve iki dikey çizginin kesişmesi ile ortaya çıkan noktalar hangi isimle anılır?",
        options: [
            "Gri noktalar",
            "Merkez noktalar",
            "Altın noktalar",
            "Asimetrik noktalar",
            "Perspektif oluşturan noktalar"
        ],
        answer: "Altın noktalar"
    }
];